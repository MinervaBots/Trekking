        ...
        if isinstance(component_type, _GenericAlias): #issubclass(component_type, List):
            for component in self._container.registry_map:
                ...
                if component_type is List[component]:
                    registration = self._container.registry_map[component]
                    result = list()
                    for register in registration:
                        result.append(register.create(self, kwargs))
                    return result
        
        # normal component
        elif component_type not in self._container.registry_map:
            raise DependencyResolutionError(
                "The requested type %s was not found in the container. Is it registered?" % component_type.__name__)

        # Independente de várias dependencias serem registradas, caso o argumento não seja uma lista
        # Retorna só a ultima dependencia registrada
        registration = self._container.registry_map[component_type]
        return registration[-1].create(self, kwargs)
...
class ContainerBuilder(object):
    ...
    def _register(self, class_type, registration, register_as):
        ...
        for available_as in register_as:
            if available_as not in self.registry:
                self.registry[available_as] = list()
                #print(register_as)
            self.registry[available_as].append(registration)
