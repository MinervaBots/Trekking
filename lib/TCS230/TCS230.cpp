...
Color TCS230::getInput()
{
  /*
  [TODO]
  Pra que a saída realmente seja RBGA (ou RGBW), temos que normalizar esse
  valores entre 0 e 255.
  */
  m_ColorCache.setRed(getRed());
  m_ColorCache.setGreen(getGreen());
  m_ColorCache.setBlue(getBlue());
  m_ColorCache.setAlpha(getWhite());
  return m_ColorCache;
}
